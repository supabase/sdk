...
- `.bun-version` — each JavaScript package pins its bun version in this file, and every workflow and composite action installs with `bash -s "bun-v$(cat <path>/.bun-version)"` rather than taking whatever is latest. The path is `.bun-version` where a `working-directory` default already points at the package, and `_sdk-spec/packages/capability-matrix/.bun-version` in the compliance actions, which run from a consumer repo's workspace root. Bumping the file is the whole upgrade; nothing else carries a version string

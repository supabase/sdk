...
- `dependabot.yml` — both JavaScript packages commit a `bun.lock`, so a single `bun` entry globs `/packages/*`. Do not move them back to the `npm` ecosystem: it rewrites `package.json` without touching `bun.lock`, which leaves the lockfile stale and fails every `bun install --frozen-lockfile` step. `oxfmt` is ignored there on purpose: postgrest-typegen formats its generated output with it, so a bump changes generated code and must be done manually in both packages together

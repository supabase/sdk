...
- `.bun-version` — each JavaScript package pins its bun version in this file, read by `oven-sh/setup-bun` (SHA-pinned, the same convention supabase-js uses) through its `bun-version-file` input. The action resolves the path from `GITHUB_WORKSPACE`, so it is `packages/<name>/.bun-version` in this repo's own workflows and `_sdk-spec/packages/capability-matrix/.bun-version` in the compliance actions, which run from a consumer repo's workspace root. A missing file only warns and silently falls back to latest, so move the path whenever the checkout layout changes. Bumping the file is the whole upgrade; nothing else carries a version string

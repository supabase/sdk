...
- `scripts/dart_symbol_extractor/` (sibling Dart package) — Small `package:analyzer` tool that walks `lib/**.dart` syntactically and emits the same `ParseResult` JSON; invoked via the `parse-dart` npm script. Parses without `pub get`; supports extension types and enhanced enums

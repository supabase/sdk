...
describe("normalize — class", () => {
  ...
  it("emits class method", () => {
    const result = normalize(
      project(
        cls("AuthClient", "src/auth.ts", method("signUp", "src/auth.ts")),
      ),
    );
    expect(result.symbols).toContainEqual({
      name: "AuthClient.signUp",
      kind: "method",
      file: "src/auth.ts",
    });
  });
  ...
  it("emits class property", () => {
    const result = normalize(
      project(cls("AuthClient", "src/auth.ts", prop("session", "src/auth.ts"))),
    );
    expect(result.symbols).toContainEqual({
  ...
  it("emits accessor as method kind", () => {
    const result = normalize(
      project(
        cls("AuthClient", "src/auth.ts", accessor("token", "src/auth.ts")),
      ),
    );
    expect(result.symbols).toContainEqual({
      name: "AuthClient.token",
      kind: "method",
      file: "src/auth.ts",
    });
  });
...
describe("normalize — interface", () => {
  ...
  it("emits interface members as property", () => {
    const result = normalize(
      project(
        iface("Session", "src/session.ts", prop("user", "src/session.ts")),
      ),
    );
    expect(result.symbols).toContainEqual({
      name: "Session.user",
...
describe("normalize — enum", () => {
  ...
  it("emits enum member as property kind", () => {
    const result = normalize(
      project(
        enumDecl("UserRole", "src/role.ts", enumMember("Admin", "src/role.ts")),
      ),
    );
    expect(result.symbols).toContainEqual({
      name: "UserRole.Admin",
...
describe("normalize — top-level declarations", () => {
  ...
  it("emits type alias as variable kind", () => {
    const result = normalize(
      project(typeAlias("AuthResponse", "src/types.ts")),
    );
    expect(result.symbols).toContainEqual({
      name: "AuthResponse",
      kind: "variable",
  ...
  it("skips Reference kind", () => {
    const result = normalize(
      project(cls("AuthClient", "src/auth.ts"), ref("Client")),
    );
    const names = result.symbols.map((s) => s.name);
...
describe("normalize — traversal", () => {
  it("walks into Module wrapper (kind 2)", () => {
    const result = normalize(
      project(mod("src/auth", cls("AuthClient", "src/auth.ts"))),
    );
    expect(result.symbols).toContainEqual({
      name: "AuthClient",
      kind: "class",
  ...
  it("walks into Namespace wrapper (kind 4)", () => {
    const ns = {
      ...
      name: "Utils",
      flags: {},
      children: [fn("helper", "src/utils.ts")],
    };
...
describe("normalize (fixture — real TypeDoc 0.27 output)", () => {
  ...
  it("finds AuthResponse type alias as variable kind", () => {
    const sym = normalize(fixture).symbols.find(
      (s) => s.name === "AuthResponse",
    );
...
describe("mergeProjects", () => {
  ...
  it("concatenates symbols from multiple projects", () => {
    ...
    const b = project(
      cls("StorageClient", "packages/core/storage-js/src/index.ts"),
    );
  ...
  it("preserves each project's (repo-relative) file paths", () => {
    ...
    expect(merged.symbols.find((s) => s.name === "AuthClient")?.file).toBe(
      "packages/core/auth-js/src/index.ts",
    );
    expect(merged.symbols.find((s) => s.name === "StorageClient")?.file).toBe(
  ...
  it("keeps duplicate re-exported names (deduped by name downstream)", () => {
    ...
    const fns = project(
      cls("FunctionsClient", "packages/core/functions-js/src/index.ts"),
    );
    const sb = project(
      cls("FunctionsClient", "packages/core/functions-js/dist/types.d.ts"),
    );
    const names = mergeProjects([fns, sb])
      .symbols.map((s) => s.name)
      .filter((n) => n === "FunctionsClient");

...
4. **Resolve the key.** The `kid` is looked up in the cached JWK Set. An unknown `kid` forces a refetch even when the cache is fresh, so freshly rotated signing keys are usable immediately. SDKs should rate-limit these forced refetches with exponential backoff: while consecutive forced fetches fail to resolve the `kid` that forced them, double the wait before the next one (for example one second up to a sixteen-second backstop), and clear the wait once a fetch resolves its `kid`. Misses inside the wait resolve as unknown and take the step 3 fallback, so a storm of attacker-minted `kid`s decays to a bounded fetch rate while a genuine rotation still verifies - immediately when the backoff is clear, via the fallback during a storm. If the refetched document still lacks the `kid`, fall back to server-side verification as in step 3.

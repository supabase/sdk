...
-- Table pinning the scalar types that no other fixture table uses as a column.
-- Nothing else here declares a bool, smallint, real or double precision column,
-- so Swift never generated Bool, Int16, Float or Double and no test asserted
-- them. Swift and Go both map the integer and float families to distinct widths
-- (Int16/Int32/Int64 and Float/Double, int16/int32/int64 and float32/float64),
-- so both goldens gain from these columns; Python collapses each family onto
-- int and float and is unaffected either way.
CREATE TABLE public.scalar_type_test (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
INSERT INTO public.scalar_type_test (flag, small_int, single_precision, double_precision)
VALUES
  (true, 1, 1.5, 2.5),
...
-- A second schema reusing names from public: a same-named enum, a same-named
-- table, and a foreign key into public, so type_schema and cross-schema
-- relationships are exercised.
CREATE SCHEMA inventory;

CREATE TYPE inventory.user_status AS ENUM ('STOCKED', 'DISCONTINUED');

CREATE TABLE inventory.users (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  status inventory.user_status DEFAULT 'STOCKED',
  public_user_id bigint REFERENCES public.users (id)
);

CREATE TABLE inventory.items (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  owner_id bigint NOT NULL REFERENCES inventory.users (id),
  status inventory.user_status NOT NULL DEFAULT 'STOCKED'
);
